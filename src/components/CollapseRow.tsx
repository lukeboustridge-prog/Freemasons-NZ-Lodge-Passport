...
export function Chevron({ open }: { open: boolean }) {
  return (
    <svg className={\`w-4 h-4 transition-transform \${open ? "rotate-90" : ""}\`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
export function RowShell({ header, actions, children, open, onToggle }: { header: React.ReactNode; actions?: React.ReactNode; children: React.ReactNode; open: boolean; onToggle: () => void; }) {
  return (
    <div className="rounded-xl border bg-white overflow-hidden">
      <button type="button" onClick={onToggle} className="w-full flex items-center justify-between px-3 py-2 text-left">
